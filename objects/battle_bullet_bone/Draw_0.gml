...
if(!no_bottom){
	draw_sprite_ext(bonetop,0,x,y,yscale,xscale,dir,color,alpha);
	draw_sprite_ext(spr_bone_body,0,x+lengthdir_x(topl,dir),y+lengthdir_y(topl,dir),length-2*topl,xscale,dir,color,alpha);
	draw_sprite_ext(bonetop,0,x+lengthdir_x(length,dir),y+lengthdir_y(length,dir),-yscale,xscale,dir,color,alpha);
}else{
